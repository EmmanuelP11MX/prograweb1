...
<form class="container-fluid" method="POST" action="proyecto.php?action=<?php echo $action; ?>" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="date" name="data[fecha_inicial]" class="form-control" placeholder="Fecha de inicio" value="<?php echo isset($data[0]['fecha_inicial']) ? $data[0]['fecha_inicial'] : ''; ?>" required minlength="3" maxlength="200" />
    </div>
    <div class="mb-3">
        <label class="form-label">Fecha final</label>
        <input type="date" name="data[fecha_final]" class="form-control" placeholder="Fecha de fin" value="<?php echo isset($data[0]['fecha_final']) ? $data[0]['fecha_final'] : ''; ?>" required minlength="3" maxlength="200" />
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <?php endif; ?>
        <input type="file" name="archivo" class="form-control" value='<?php echo isset($data[0]['archivo']) ? $data[0]['archivo'] : ''; ?>' />
    </div>
    <div class="mb-3">
        <?php if ($action == 'edit') : ?>
            <input type="hidden" name="data[id_proyecto]" value="<?php echo isset($data[0]['id_proyecto']) ? $data[0]['id_proyecto'] : ''; ?>">
        <?php endif; ?>
